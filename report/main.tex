...
Además, este shaping incorpora bonus discretos al alcanzar regiones relevantes del espacio de estados. Estos bonus permiten reforzar de forma más clara la consecución de hitos intermedios, lo que ayuda a acelerar la convergencia.
...
Aunque en las secciones anteriores se ha observado que el enfoque multimodal junto con el uso de curriculum learning permite obtener políticas razonables, todavía no está claro qué componentes del sistema son realmente los responsables de esta mejora. Es decir, no basta con comprobar que el modelo completo funciona, sino que resulta necesario entender qué aporta cada una de sus partes.

Con este objetivo, se plantea un estudio de ablación en el que se analizan de forma separada tres elementos clave del proyecto: el uso del reward shaping basado en curriculum, la información tabular propioceptiva y la información visual. Este análisis permite estudiar hasta qué punto el rendimiento final depende de cada uno de estos factores y si realmente todos ellos son necesarios.

\subsection{Configuración experimental}

Para que la comparación sea lo más limpia posible, todas las variantes se entrenan manteniendo los mismos hiperparámetros y el mismo entorno, modificando únicamente los componentes que se quieren analizar. En concreto, se consideran las siguientes configuraciones:

\begin{itemize}
    \item \textbf{Multimodal con curriculum}: modelo completo, que utiliza imágenes, features tabulares y reward shaping progresivo.
    \item \textbf{Multimodal sin curriculum}: misma arquitectura multimodal, pero sin el esquema de curriculum en la recompensa.
    \item \textbf{Solo features}: el agente recibe únicamente observaciones tabulares.
    \item \textbf{Solo imágenes}: el agente recibe únicamente observaciones visuales.
\end{itemize}

Todas las variantes se entrenan durante 300\,000 pasos y se evalúan utilizando una misma recompensa común, de forma que la comparación no dependa de la escala de la reward utilizada durante el entrenamiento.

\subsection{Resultados y discusión}

Para analizar los resultados no se ha considerado únicamente la recompensa final, sino también la evolución de la recompensa a lo largo del entrenamiento. Esto permite entender mejor cómo aprende cada modelo, si lo hace de forma estable y si la mejora es progresiva o depende de comportamientos puntuales.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{figuras/ablation_eval_and_training.png}
...
En la parte superior de la Figura \ref{fig:ablation_curves} se muestra la recompensa de evaluación, que es la métrica más relevante para comparar las distintas configuraciones. Se observa claramente que la variante \textit{multimodal sin curriculum} es la que obtiene mejor rendimiento de forma consistente durante prácticamente todo el entrenamiento. Su curva es además bastante estable, con pocas oscilaciones, lo que indica que el aprendizaje es robusto.

Por otro lado, la variante \textit{solo features} alcanza resultados razonables y se sitúa como la segunda mejor configuración. Aunque no llega al nivel de la mejor variante, mantiene un rendimiento relativamente estable y claramente superior al caso de solo imágenes. Esto sugiere que la información tabular contiene gran parte de la señal necesaria para resolver la tarea.

La variante \textit{multimodal con curriculum} presenta un comportamiento más irregular. En algunos momentos del entrenamiento se aproxima al rendimiento de \textit{solo features}, pero no consigue superarlo de forma consistente y termina con un resultado ligeramente inferior. Además, se observan más oscilaciones en la curva, lo que indica una mayor inestabilidad.

Finalmente, la variante \textit{solo imágenes} queda claramente por debajo del resto durante todo el entrenamiento. No se observa una mejora significativa en ningún momento, lo que indica que, bajo las condiciones de esta práctica, la información visual por sí sola no es suficiente para aprender una política útil.

La parte inferior de la figura muestra la recompensa de entrenamiento, y aquí aparece un comportamiento interesante. La variante \textit{solo features} alcanza valores muy altos de recompensa durante el entrenamiento, incluso superiores a los del modelo multimodal. Sin embargo, esta ventaja no se traduce completamente en la evaluación final. Algo similar ocurre con la variante \textit{multimodal con curriculum}, que también alcanza recompensas de entrenamiento elevadas pero no consigue el mejor rendimiento en evaluación.

En cambio, la variante \textit{multimodal sin curriculum}, que es la mejor en evaluación, presenta valores de recompensa de entrenamiento mucho más modestos y bastante planos. Esto indica que optimizar más la reward durante el entrenamiento no implica necesariamente obtener una mejor política final cuando se evalúa con una métrica común.

Este resultado es especialmente relevante, ya que sugiere que el reward shaping introduce una señal de aprendizaje que puede ser útil para guiar la exploración, pero que no siempre está perfectamente alineada con el objetivo final de la tarea.

Para completar el análisis, en la Figura \ref{fig:ablation} se muestra un resumen de la evaluación final de cada variante.
...
Esta figura confirma las tendencias observadas en las curvas: la mejor configuración es la multimodal sin curriculum, seguida de la variante solo features, mientras que el uso exclusivo de imágenes produce los peores resultados. Además, la diferencia entre variantes es suficientemente grande como para que las conclusiones sean claras.

\subsection{Conclusiones de la Parte 3}

El estudio de ablación permite extraer varias conclusiones importantes. En primer lugar, la información tabular resulta ser fundamental para el aprendizaje de la tarea, ya que incluso sin el canal visual el agente es capaz de aprender políticas razonables. En segundo lugar, el uso exclusivo de imágenes no es suficiente bajo las limitaciones de resolución y computación de esta práctica.

Por otro lado, la combinación multimodal sí aporta beneficios, pero el uso de curriculum learning en la recompensa no parece mejorar el rendimiento final en este caso. De hecho, la mejor configuración es la que utiliza información multimodal sin aplicar shaping progresivo.
