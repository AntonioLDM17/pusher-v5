...
Para mantener el coste computacional dentro del tiempo disponible, los experimentos de ablación se proponen sobre un número reducido de pasos de entrenamiento, manteniendo el resto de hiperparámetros fijos. En concreto, se consideran las siguientes variantes:

\begin{itemize}
    \item \textbf{Sin curriculum reward}: se elimina el shaping progresivo de la recompensa y se entrena al agente sin la interpolación entre \textit{reach} y \textit{push}.
...
Estas tres configuraciones permiten aislar la contribución de la recompensa diseñada y de las dos fuentes principales de información utilizadas en el modelo híbrido.

Siempre que sea posible, estas variantes se evaluarán no solo mediante recompensa acumulada, sino también mediante episodios de evaluación visual y análisis cualitativo del comportamiento, siguiendo el mismo procedimiento utilizado para el modelo principal.

\textbf{TODO: especificar aquí el número exacto de timesteps usado en las ablaciones (por ejemplo 20\,000, 50\,000 o 100\,000).}

\subsection{Resultados esperados y discusión}

Desde el punto de vista conceptual, este estudio de ablación permite validar varias de las hipótesis planteadas a lo largo del trabajo. En primer lugar, si la versión sin curriculum reward empeora claramente, se refuerza la idea de que la recompensa diseñada ayuda a estructurar la exploración del agente. En segundo lugar, si la variante solo imágenes vuelve a mostrar un rendimiento pobre, ello confirmará que el canal visual, por sí solo, no resulta suficiente bajo las restricciones computacionales de esta práctica. Finalmente, si la variante solo features aprende mejor que la variante solo imágenes, podrá concluirse que gran parte de la señal útil de control se encuentra en la información tabular.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{figuras/ablation_comparison.png}
    \caption{Comparación de las distintas configuraciones del estudio de ablación.}
    \label{fig:ablation}
\end{figure}

\textbf{TODO: sustituir este texto por resultados reales en cuanto termine la ablación y esté disponible la figura definitiva \texttt{ablation\_comparison.png}.}

\textbf{TODO: si da tiempo, añadir también curvas de aprendizaje comparadas para baseline, sin curriculum, solo features y solo imágenes.}
